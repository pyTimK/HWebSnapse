...
import HomogenizeForm from "./components/forms/HomogenizeForm";
...
function App() {
  ...
  const [showHomogenizeModal, setShowHomogenizeModal] = useState(false);
  ...
  const handleCloseHomogenizeModal = () => setShowHomogenizeModal(false);
  const handleShowHomogenizeModal = () => setShowHomogenizeModal(true);
  ...
  //! ADDED TO HANDLE HOMOGENIZAION OF AN SN P SYSTEM
  const handleHomogenize = (scaling_type) => {
    //Convert JSON Array to string.
    var wrapper = { content: neurons };
    var result = convert.json2xml(wrapper, options);
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({ scaling_type, snp_system: result }),
    };

    fetch(homogenization_api_url, requestOptions)
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
        const errors = data.errors;
        const homogenized_snp_system = data.homogenized_snp_system;
        console.log(homogenized_snp_system);

        if (
          errors != null ||
          homogenized_snp_system == null ||
          homogenized_snp_system == ""
        ) {
          console.log("HAS HOMOGENIZATION ERRORS");
          console.log(errors);
        } else {
          ...
          const result = convert.xml2js(homogenized_snp_system, options);
          console.log("ssssssssssssresult");
          console.log(result);
      ...
      <Switch>
        <Route path="/">
          <Container>
            ...
            <div>
              ...
              <Row>
                ...
                <Col>
                  ...
                  >
                    <Button variant="link" onClick={() => onBackward()}>
                      <SkipBackwardFill />
                    </Button>{" "}
                    ...
                    <Button variant="link" onClick={() => onForward()}>
                      <SkipForwardFill />
                    </Button>{" "}
                    <Button
                      hidden={true}
            ...
            />
            <HomogenizeForm
              showHomogenizeModal={showHomogenizeModal}
              handleCloseHomogenizeModal={handleCloseHomogenizeModal}
              handleHomogenize={handleHomogenize}
              handleError={showError}
            />
            <DeleteAllForm
